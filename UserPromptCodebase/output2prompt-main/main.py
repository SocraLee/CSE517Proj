import os
os.environ["OPENAI_API_KEY"] = ""# put your openai api here for CS score computation.
...
def train(dataset_path, number_of_outputs = 64):
    ...
    config_dict['eval_steps'] = 3
    config_dict['experiment'] = 'inversion_from_output_sparse'
    config_dict["num_train_epochs"] = 3
    ...
    name = f"train{number_of_outputs}_prompt2output_{mode}"
    ...
    print("New noo:{}".format(len(train_ds[0]['result_list'])))
...
def test(model_path, dataset_path,number_of_outputs = 64):
    with open('prompt2output/config.json') as f:
        config_dict = json.load(f)
    config_dict['use_wandb'] = False
    config_dict['report_to'] = []
    ...
    mode = 'just_sparse'
    print(mode)
    name = f"test_prompt2output_{mode}"
    experiment: experiments.InversionFromOutputSparseExperiment = analyze_utils.load_experiment_from_config(
        name, config=config, use_less_data = -1
    )
    experiment.exp_name = name
    ...
    trainer.enable_emb_cos_sim_metric() # use this to activate cs metrics. Note that you have to configure your own OpenAI account API_Key setting to use acquire embeddings.
    trainer.tokenizer = tokenizer
    trainer.embedder_tokenizer = tokenizer
    trainer.args.metric_for_best_model = None
...
inverters = {
    ...
    "user_prompts_lm":"saves/train64_prompt2output_just_sparse_2025-03-11_16-18-59/checkpoint-10800"
}
...
if __name__ == '__main__':
    ...
    if len(sys.argv) > 4:
        ...
        number_of_outputs = 64
    print("number of outputs:",number_of_outputs)
    if mode == 'train':
        train(dataset_dict[dataset][0], number_of_outputs)
